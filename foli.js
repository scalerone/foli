...
if(lD==1){	cM=cyclical((y-1900)*12+m+13);}
//日柱
...
//节气
tmp1=sTerm(y,m*2  )-1;	tmp2=sTerm(y,m*2+1)-1;
//强制挪移立春
if( y==2013 && m==1){
	if(tmp1==2) tmp1=3;
	this[2].lYear--;
...
this[tmp1].solarTerms = solarTerm[m*2];		this[tmp2].solarTerms = solarTerm[m*2+1];
//guohao
if( y==2009 && m==1){	if(tD==3){	this[tmp1].solarTerms = '';	}else if(tD==4){		this[tmp1].solarTerms = '立春';	} }
...
case 20:s = '二十'; break;
case 30:s = '三十'; break;
default :s = nStr2[Math.floor(d/10)];s += nStr1[d%10];break;
}return s;
}

///////////////////////////////////////////////////////////////////////////////

var cld,RLChangesD=0,RLChangesDay=0,RLShengxiao;

function drawCld(objid,SY,SM) {
	var i,sD,s,size;	cld = new calendar(SY,SM);
	var cld0 ;	if(SM>0) cld0=new calendar(SY,SM-1);else cld0= new calendar(SY-1,11);
...
//	sObj.style.color = cld[sD].color; //法定假日颜色
	...
	if(s.length>0) {
		...
		s=cld[sD].solarTerms;	if(s.length>0) s = s.fontcolor('limegreen');
	}
	}
...
//	s=cld[sD].sMonth+":"+cld[sD].sDay;
	...
	else {	sObj.innerHTML = '';	lObj.innerHTML = '';	}//非日期
	}
	//填充forceSolarTerms
	var jieqijia=0,jieqitxt='';
	for(i=cld0.length+cld0.firstWeek-1;i>0;i--){
		sD = i - cld0.firstWeek; if(cld0[sD].solarTerms==''){ jieqijia++; continue;	}
		jieqitxt=cld0[sD].solarTerms;	break;
	}
	for(i=0;i<42;i++){
		sD = i - cld.firstWeek;	if(sD>-1 && sD<cld.length) {
			if(cld[sD].solarTerms=='') jieqijia++; else{ jieqijia=0;jieqitxt=cld[sD].solarTerms;	}
...
function changeCld(objid) {
	var y,m;	y=$RL(objid+"_sy").selectedIndex+1900;	m=$RL(objid+"_sm").selectedIndex;	drawCld(objid,y,m);
}

function pushBtm(objid,K) {	switch (K){
	case 'YU' :if($RL(objid+"_sy").selectedIndex>0) $RL(objid+"_sy").selectedIndex--;break;
	case 'YD' :if($RL(objid+"_sy").selectedIndex<150)$RL(objid+"_sy").selectedIndex++;break;
case 'MU' :if($RL(objid+"_sm").selectedIndex>0) {$RL(objid+"_sm").selectedIndex--;}
		else {$RL(objid+"_sm").selectedIndex=11;if($RL(objid+"_sy").selectedIndex>0) $RL(objid+"_sy").selectedIndex--;}break;
	case 'MD' :if($RL(objid+"_sm").selectedIndex<11) {$RL(objid+"_sm").selectedIndex++;}
		else {$RL(objid+"_sm").selectedIndex=0;if($RL(objid+"_sy").selectedIndex<150) $RL(objid+"_sy").selectedIndex++;}break;
	default :	$RL(objid+"_sy").selectedIndex=tY-1900;	$RL(objid+"_sm").selectedIndex=tM;	RLChangesD=RLChangesDay=0;break;
	}	changeCld(objid);	mOvr(objid,RLChangesD,false);
}
...
//显示详细日期资料
function mOvr(objid,v,isover) {	var s,jieri,so,sObj=$RL(objid+"_sd"+v),d=sObj.innerHTML-1;	if(sObj.innerHTML!='') {
	RLChangesDay=cld[d].sDay;
//sYear,sMonth,sDay,week,
//lYear,lMonth,lDay,isLeap,
//cYear,cMonth,cDay
	var sxAnimals='…',sfu="";
	for(var i=0;i<Zhi.length;i++){		if(cld[d].cYear.indexOf(Zhi[i])>-1){ sxAnimals=Animals[i];	break;	}	}
	s= '<div class="rltext">' +
	...
	var jd=(cld[d].forceSolarTerms+":").split(':');	var jdindex=parseInt(jd[1]);
	...
	sfu= '<div style="background:#784b20">' +
		...
		'</font></div><div class="rlfujie">'+cld[d].solarTerms + ' ' + cld[d].solarFestival + ' ' + cld[d].lunarFestival+'</div>'
	+'</div>';
	...
	$(sObj).mouseout(function(){	$("#"+objid+"_info").hide();	});
	$("#"+objid+"_bdy").mouseout(function(){	$("#"+objid+"_info").hide();	});
...
//	$("#"+objid+"_info").stop();
	...
	for(i=0;i<42;i++) {		so=$RL(objid+"_sd"+i);	so.className=so.className.replace(' this','');	}
...
function LoadRili(objid){
	...
	drawCld(objid,tY,tM);	pushBtm(objid,'');
}
function $RL(idorname ,num ){
	...
	var idx=idorname.indexOf(',');	if(idx==-1) return document.getElementById(idorname);
