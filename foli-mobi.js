...
function drawCld(objid, SY, SM) {
    ...
    $RL(objid + "_gz").innerHTML = xDisplay + '年 ';
}
...
function mOvr(objid, v, isover) {
    ...
    if (sObj.innerHTML != '') {
        ...
        //     $riliInfo.hide();
        // });
        $('.rili_fu_close').one('click', function() {
            $riliInfo.hide();
        });
    }
}
...
function LoadRili(objid) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j < 7; j++) {
            gNum = i * 7 + j;
            rt += "<td id='" + objid + "_gd" + gNum + "' onclick='mOvr(\"" + objid + "\"," + gNum + ",true)'><div class='foli-relative'><a id='" + objid + "_container" + gNum + "' href='javascript:void(0)'><span class='rlri' id='" + objid + "_sd" + gNum + "'";
